...
The methods used to implement this algorithm are the same as the the FCFS methods. However, these methods now receive a sorted array of burst times
so as to allow the shortest jobs to execute first. 
...
The input is placed into a "Process" constructor called "PS" along with a randomly generated arrival time, which is then placed in a "Process" ArrayList. 
There are two methods besides the main method in this java file, which calculate the wait and turn-around times. There is also a CustomComparator class that will do the comparison of arrival times  
...
There are no methods besides the main method which runs the whole algorithm. There is a variable which holds the time quantum.
